...
<x-admin-layout title="Report #{{ $report->id }}" :breadcrumbs="$breadcrumbs">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <form action="{{ route('reports.assign-caretaker', $report->id) }}" method="POST" class="space-y-3">
                        @csrf
                        @method('PATCH')
    ...
    <div id="deleteConfirmModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg shadow-xl max-w-md w-full" onclick="event.stopPropagation()">
            ...
            {{-- Footer --}}
            <div class="bg-gray-50 p-4 border-t border-gray-200 flex gap-3 rounded-b-lg">
                <button type="button"
                        onclick="closeDeleteModal()"
                        class="flex-1 px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white font-medium rounded text-sm">
                    Cancel
                </button>
                ...
                <form action="{{ route('reports.destroy', $report->id) }}" method="POST" class="flex-1" id="deleteReportForm">
                    ...
                    <button type="submit"
                            id="confirmDeleteBtn"
                            class="w-full px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded text-sm flex items-center gap-2 justify-center">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
